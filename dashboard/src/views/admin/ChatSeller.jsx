...
// Seller List Component for Mobile
const SellersList = ({ sellers, activeSeller, sellerId, onClose }) => (
        ...
        <div className="px-2 lg:px-7 py-5">
            <div className="w-full bg-[#f0f2f5] rounded-lg shadow-md h-[calc(100vh-140px)] overflow-hidden">
                <div className="flex w-full h-full relative">
                    {/* Mobile Sellers List - Slide Over */}
                    <div 
                        className={`absolute md:relative md:w-[280px] w-full h-full z-50 transform transition-transform duration-300 ease-in-out ${
                    ...
                    <div className="flex-1 flex flex-col bg-white md:bg-transparent">
                        {sellerId ? (
                            <>
                                <div className="p-4 border-b border-gray-300 flex justify-between items-center bg-white">
                                    <div className="flex items-center">
                                        <button 
                                            onClick={() => navigate('/admin/dashboard/chat-sellers')}
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                {activeSeller.some(a => a.sellerId === sellerId) ? 'Online' : 'Offline'}
                                            </p>
                                        </div>
                                    </div>
                                    <button 
                                    ...
                                    {seller_admin_message.map((m, i) => (
                                        <div key={i} className={`flex ${m.senderId === sellerId ? 'justify-start' : 'justify-end'} mb-4`}>
                                            <div className={`max-w-[85%] md:max-w-[70%] p-3 rounded-lg ${
                                                m.senderId === sellerId ? 'bg-white text-gray-800' : 'bg-blue-500 text-white'
                                            }`}>
                                                {m.message}
                                            </div>
                                        </div>
                            ...
                            <div className="flex-1 flex items-center justify-center flex-col text-gray-500 bg-white">
                                <button 
                                    onClick={() => setShow(true)}
                                    className="md:hidden mb-4 text-blue-500 flex items-center"
                                >
